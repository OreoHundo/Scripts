...
if Event == "Christmas2024" and Christmas2024_Settings then
	Christmas2024_BuyBattlePass = Christmas2024_Settings.BuyBattlePass
	Christmas2024_BuyCrates = Christmas2024_Settings.BuyCrates
	Christmas2024_Webhook_Frequency = Christmas2024_Settings.Frequency
	Christmas2024_Webhook_URL = Christmas2024_Settings.SnowTokens_URL
	Christmas2024_FinishedWebhook_URL = Christmas2024_Settings.Finished_URL
	warn("Christmas2024 Settings", Christmas2024_Settings)
end
...
if isfile(Christmas2024_CandyFile) then
	Christmas2024_MessageID = readfile(Christmas2024_CandyFile)
end
...
local function WebhookSend(TaiShii, Name, Rarity, Color)
	...
	if Event == "Christmas2024" then
		...
		elseif TaiShii == "Start" then
			if Christmas2024_MessageID ~= "" then
				print("existing")
				JSONStuff = [[
				{
				  "content": "Account: ]]..tostring(LocalPlayer.Name)..[[",
			...
			local Response = request({
				Url = Christmas2024_Webhook_URL .. "?wait=true",
				Method = "POST",
				Body = JSONStuff,
				Headers = {["Content-Type"] = "application/json"}
			})
			...
			Christmas2024_MessageID = ID
			writefile(Christmas2024_CandyFile, Christmas2024_MessageID)
		end
	end
		...
		while MapConnections[Child.Name] == true do
			...
			if FullCoinBag and not FullCoinBag.Visible then
				...
				elseif Event == "Christmas2024" then
					...
					warn("Christmas2024", math.ceil(ClosestDist), "Closest Coin")
					if ClosestDist < MaxDist and Closest then 
						local wft = TPTween(Closest.Position)
					end
